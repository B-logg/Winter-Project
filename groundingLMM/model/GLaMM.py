...
class GLaMMForCausalLM(LlavaLlamaForCausalLM):
    ...
    def _create_seg_token_mask(self, input_ids):
        ...
        return torch.cat(
            [torch.zeros((mask.shape[0], 575)).bool().to(self.device), mask, torch.zeros((mask.shape[0], 1)).bool().to(self.device)],
            dim=1
        )
    ...
    def evaluate(self, global_enc_images, grounding_enc_images, input_ids, resize_list, orig_sizes, max_tokens_new=32,
        ...
        with torch.no_grad():
            ...
            seg_token_mask = torch.cat(
                [torch.zeros((seg_token_mask.shape[0], 575), dtype=torch.bool).to(self.device), seg_token_mask], dim=1, )
            # Process hidden states
