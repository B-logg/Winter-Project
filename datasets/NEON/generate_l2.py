...
def generate_l2_qa(species_counts):
    ...
    사용자의 질문 하나에 대해, 위 상황의 모든 생태적 정보를 하나의 종합적인 답변으로 압축하여 JSON을 생성하세요.
    절대로 리스트([])를 만들지마세요. 단 하나의 객체({{}})만 출력하세요.
    
    [규칙]
    - 답변에서 언급된 수종(침엽수 or 활엽수) 뒤에는 반드시 [SEG] 토큰을 붙여야 합니다.
    - 침엽수, 활엽수 언급 순서는 무조건 침엽수를 먼저 언급하고, 이후에 활엽수를 언급하고, 실제 존재하는 수종만 언급하세요.
    - 질문은 "이 숲의 구성은?", "어떤 나무들이 있어?", "나무들의 관계를 설명해줘" 등 다양하게 생각하고, 한 타일 당 딱 한 문장만 사용해
    - 질문과 답변은 한 타일 당 한국어로 한 문장 씩으로 해주고, 최대한 예시 답변 패턴, 예시 정보 처럼 산림에 대한 다양한 정보를 추가해줘.
    - 답변은 처음에 이미지에서 보이는 침엽수, 활엽수의 구성이나 관계, 위치를 설명하고, 이후에 산림 전체와 침엽수, 활엽수의 관계를 설명하고, 추가적인 설명이 들어가도록 해줘.
    ...
    [출력 포맷]
    {{
        "question": "이 숲의 생태적 특성과 현황에 대해 종합적으로 설명해줘.", 
        "answer": "이 숲은 ... 침엽수 [SEG] ... 활엽수 [SEG] ... (모든 정보가 압축된 긴 답변)"}}
    """
